...
VISIBILITY_RADIUS_VALUES = [-1, 15]  # Full visibility vs limited visibility
REWARD_TYPE_FOR_FIG9 = "potential_field"

def main_fig9_trainer(args):
    """Main function to orchestrate training for Figure 9 models (visibility radius comparison)."""
    ...
    wandb_project_for_fig9 = os.getenv("WANDB_PROJECT_FIG9", "sirs-rl-fig9-visibility")

    for visibility_radius in VISIBILITY_RADIUS_VALUES:
        print(f"\n=== Training models for Figure 9 (Visibility Radius: {visibility_radius}, Reward: {REWARD_TYPE_FOR_FIG9}) ===")
        
        env_config_for_this_run = copy.deepcopy(initial_env_config)
        env_config_for_this_run['visibility_radius'] = visibility_radius
        env_config_for_this_run['reward_type'] = REWARD_TYPE_FOR_FIG9
        ...
        base_run_name_for_group = f"Fig9_{visibility_str}"
        if args.exp_suffix:
        ...
        for seed_val in SEEDS_FOR_TRAINING:
            ...
            print(f"# Preparing training for: {seed_specific_run_name} (Visibility Radius: {visibility_radius}, Seed: {seed_val}) #")
            ...
            execute_single_training_run(
                ...
                wandb_project_name=wandb_project_for_fig9,
                wandb_group_name=base_run_name_for_group
            )
            print(f"# Completed training for: {seed_specific_run_name} #")
        print(f"=== Finished all seeds for Visibility Radius: {visibility_radius} ===")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train models with different visibility radius settings for Figure 9.")
